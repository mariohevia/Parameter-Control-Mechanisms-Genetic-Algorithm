...
DFastLambdaLambda::DFastLambdaLambda(Random& _rand, shared_ptr<Evaluator> _evaluator,
  ...
  float Bs = 1.0;
  float us = length;
  vector<double> pmfs(us);
  std::iota(pmfs.begin(), pmfs.end(), 1);
  for(size_t i = 0; i < us; i++) {
    pmfs[i] = 1.0 / pow(pmfs[i], Bs);
  }
  pmfs.insert(pmfs.begin(), 0);
  power_dist_s = std::discrete_distribution<int>(pmfs.begin(),pmfs.end());

  float Bl = 2.0;
  float ul = std::min(ceil(pow(5, length/10.0)), 1000000.0);
  vector<double> pmfl(ul);
  std::iota(pmfl.begin(), pmfl.end(), 1);
  for(size_t i = 0; i < ul; i++) {
    pmfl[i] = 1.0 / pow(pmfl[i], Bl);
  }
  pmfl.insert(pmfl.begin(), 0);
  power_dist_lambda = std::discrete_distribution<int>(pmfl.begin(),pmfl.end());
}

// Selects "flips" number of random locations to perform bit flips.
vector<bool> DFastLambdaLambda::mutate(const vector<bool>& parent, const int flips) {
...
vector<bool> DFastLambdaLambda::crossover(const vector<bool>& p1,
...
bool DFastLambdaLambda::iterate() {
  ...
  lambda = 1.0 * power_dist_lambda(rand);
  int s = power_dist_s(rand);
  float pc = sqrt(1.0 * s / length);
  std::binomial_distribution<> binom(length, pc);
  std::bernoulli_distribution cross(pc);
  flips = binom(rand);
