...
LambdaLambdaCap::LambdaLambdaCap(Random& _rand, shared_ptr<Evaluator> _evaluator,
...
vector<bool> LambdaLambdaCap::mutate(const vector<bool>& parent, const int flips) {
...
vector<bool> LambdaLambdaCap::crossover(const vector<bool>& p1,
...
bool LambdaLambdaCap::iterate() {
  ...
  if (fitness < next_parent_fitness) {
    ...
    lambda *= 1.1067;
    int half_lenght = (int)(length/2);
    if (lambda > half_lenght) {
      lambda = half_lenght;
    }
  }
