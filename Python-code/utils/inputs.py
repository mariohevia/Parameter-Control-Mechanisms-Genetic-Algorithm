...
class CommandLine:
    def __init__(self):
        ...
        problems = ['OneMax', 'LeadingOnes', 'SufSamp', 'MakespanScheduling', 'Jump', 'Trap']
        ...
        algorithms = ['OnePlusLambda', 'OnePlusLambdaSAMut',
                      ...
                      'OnePlusLambdaSAOffImp3', 'OnePlusLambdaSAOffImp4',
        ...
        parser.add_argument('-t', dest='problem', type=str, default='OneMax',
                            #choices=problems, 
                            metavar='', help='Problem to '
                            'solve. Allowed values are: '+', '.join(problems)+
                            '. (default: %(default)s)')
        parser.add_argument('-n', dest='problem_size', type=int, metavar=
                            '[10-500000]', default=100,
                            help='Size of the problem "n". (default: '
                            '%(default)d)')
        parser.add_argument('-k', dest='jump_size', type=int, metavar=
                            '[1-500000]', default=1,
                            help='Size of the jump "k". (default: '
                            '%(default)d)')
        parser.add_argument('--extra', dest='extra', type=str, help='Anything')
        parser.add_argument('-r', dest='num_runs', type=int, metavar=
                            '[1-25000]', default=100, 
                            help='Number of runs. (default: %(default)d)')
        parser.add_argument('--stop_criteria', dest='stop_criteria', type=str, metavar=''
        ...
        parser.add_argument('-G', dest='offspring_update_factor', type=float, 
                            metavar='[1.0-100.0]', default=2, help='Offspring '
                            'size population update factor. (default: '
                            '%(default)d)')
        parser.add_argument('-s', dest='success_ratio', type=float, metavar=
                            '(0-1000]', default=1, 
                            help='Success ratio s. (default: %(default)d)')
        ...
        self.success_ratio = args.success_ratio
        self.seed = args.seed
        ...
        alg_acro = ['(1+λ) EA', 
                    ...
                    '(1+λ) EA with improved self-adjusting offspring population size 4', 
        ...
        if self.algorithm == 'help':
            ...
            parser.exit()
        if self.problem_size < 10 or self.problem_size > 500000:
            parser.print_usage()
            print('master.py: error: argument -n: invalid choice:',
                  self.problem_size, '(choose from range 10 - 500000)')
            parser.exit()
        if self.num_runs < 1 or self.num_runs > 25000:
            parser.print_usage()
            print('master.py: error: argument -r: invalid choice:',
                  self.num_runs, '(choose from range 1 - 25000)')
            parser.exit()
...
#                  self.offspring_update_factor, '(change argument -a)')
        if self.offspring_update_factor > 100 or self.offspring_update_factor < 0:
            parser.print_usage()
            print('master.py: error: argument -G: invalid choice:',
                  self.offspring_update_factor, '(choose from range 0.0 - 100.0)')
            parser.exit()
